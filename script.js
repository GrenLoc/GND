...
function initMap() {
  ...
  if (searchInput && google.maps.places) {
    const autocomplete = new google.maps.places.Autocomplete(searchInput, {
      componentRestrictions: { country: 'gd' },   // Grenada only
      fields: ['geometry', 'name', 'formatted_address'],
      types: ['geocode', 'establishment']
    });

    // Bias results toward the current map viewport
    autocomplete.bindTo('bounds', map);

    autocomplete.addListener('place_changed', function () {
      const place = autocomplete.getPlace();

      // No geometry = user hit Enter without selecting a suggestion
      ...
      // Reuse the existing handleMapClick logic by synthesising its expected arg
      handleMapClick({ latLng: place.geometry.location });

      // Clear the search input after selection for a clean UX
      searchInput.value = '';
      searchInput.blur();
    });
  }
